...
export class SousFamilleComponent implements OnInit {
  ...
  protected onSuccess(data: ISousFamille[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/sous-famille'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
