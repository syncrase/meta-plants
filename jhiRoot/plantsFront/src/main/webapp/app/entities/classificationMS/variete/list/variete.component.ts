...
export class VarieteComponent implements OnInit {
  ...
  protected onSuccess(data: IVariete[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/variete'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
