...
export class OrdreComponent implements OnInit {
  ...
  trackId(index: number, item: IOrdre): number {
    return item.id!;
  }

  delete(ordre: IOrdre): void {
    const modalRef = this.modalService.open(OrdreDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IOrdre[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
