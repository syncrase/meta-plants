...
describe('Mois e2e test', () => {
  ...
  it('should load Mois', async () => {
    ...
    expect(await moisComponentsPage.getTitle()).to.eq('Mois');
  ...
  it('should load create Mois page', async () => {
    ...
    expect(await moisUpdatePage.getPageTitle()).to.eq('Create or edit a Mois');
  ...
  it('should delete last Mois', async () => {
    ...
    expect(await moisDeleteDialog.getDialogTitle()).to.eq('Are you sure you want to delete this Mois?');
