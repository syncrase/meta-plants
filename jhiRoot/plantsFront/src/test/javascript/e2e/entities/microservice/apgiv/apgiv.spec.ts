...
describe('APGIV e2e test', () => {
  ...
  it('should load APGIVS', async () => {
    ...
    expect(await aPGIVComponentsPage.getTitle()).to.eq('APGIVS');
  ...
  it('should load create APGIV page', async () => {
    ...
    expect(await aPGIVUpdatePage.getPageTitle()).to.eq('Create or edit a APGIV');
  ...
  it('should delete last APGIV', async () => {
    ...
    expect(await aPGIVDeleteDialog.getDialogTitle()).to.eq('Are you sure you want to delete this APGIV?');
