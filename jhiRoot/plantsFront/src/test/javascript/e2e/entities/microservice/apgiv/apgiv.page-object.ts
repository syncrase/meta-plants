...
export class APGIVComponentsPage {
  ...
  deleteButtons = element.all(by.css('perma-apgiv div table .btn-danger'));
  title = element.all(by.css('perma-apgiv div h2#page-heading span')).first();
...
export class APGIVUpdatePage {
  pageTitle = element(by.id('perma-apgiv-heading'));
...
export class APGIVDeleteDialog {
  private dialogTitle = element(by.id('perma-delete-aPGIV-heading'));
  private confirmButton = element(by.id('perma-confirm-delete-aPGIV'));
