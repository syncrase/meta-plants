...
export class FormeComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('perma-forme div table .btn-danger'));
  title = element.all(by.css('perma-forme div h2#page-heading span')).first();
...
export class FormeUpdatePage {
  pageTitle = element(by.id('perma-forme-heading'));
...
export class FormeDeleteDialog {
  private dialogTitle = element(by.id('perma-delete-forme-heading'));
  private confirmButton = element(by.id('perma-confirm-delete-forme'));
