...
export class ClasseComponentsPage {
  ...
  deleteButtons = element.all(by.css('perma-classe div table .btn-danger'));
  title = element.all(by.css('perma-classe div h2#page-heading span')).first();
...
export class ClasseUpdatePage {
  pageTitle = element(by.id('perma-classe-heading'));
...
export class ClasseDeleteDialog {
  private dialogTitle = element(by.id('perma-delete-classe-heading'));
  private confirmButton = element(by.id('perma-confirm-delete-classe'));
