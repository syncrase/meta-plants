...
/**
 * Performance test for the Cronquist entity.
 */
class CronquistGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Cronquist entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cronquists")
            .get("/services/microservice/api/cronquists")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cronquist")
            .post("/services/microservice/api/cronquists")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "regne":"SAMPLE_TEXT"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created cronquist")
                .get("/services/microservice${new_cronquist_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cronquist")
            .delete("/services/microservice${new_cronquist_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
