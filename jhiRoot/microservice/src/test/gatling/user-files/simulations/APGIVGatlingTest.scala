...
/**
 * Performance test for the APGIV entity.
 */
class APGIVGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the APGIV entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all aPGIVS")
            .get("/services/microservice/api/apgivs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new aPGIV")
            .post("/services/microservice/api/apgivs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "ordre":"SAMPLE_TEXT"
                , "famille":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_aPGIV_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created aPGIV")
                .get("/services/microservice${new_aPGIV_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created aPGIV")
            .delete("/services/microservice${new_aPGIV_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
