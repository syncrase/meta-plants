...
export class CronquistComponentsPage {
  ...
  deleteButtons = element.all(by.css('gp-cronquist div table .btn-danger'));
  title = element.all(by.css('gp-cronquist div h2#page-heading span')).first();
...
export class CronquistUpdatePage {
  pageTitle = element(by.id('gp-cronquist-heading'));
...
export class CronquistDeleteDialog {
  private dialogTitle = element(by.id('gp-delete-cronquist-heading'));
