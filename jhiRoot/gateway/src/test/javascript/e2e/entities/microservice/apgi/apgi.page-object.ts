...
export class APGIComponentsPage {
  ...
  deleteButtons = element.all(by.css('gp-apgi div table .btn-danger'));
  title = element.all(by.css('gp-apgi div h2#page-heading span')).first();
...
export class APGIUpdatePage {
  pageTitle = element(by.id('gp-apgi-heading'));
...
export class APGIDeleteDialog {
  private dialogTitle = element(by.id('gp-delete-aPGI-heading'));
  private confirmButton = element(by.id('gp-confirm-delete-aPGI'));
