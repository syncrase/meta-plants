...
export class TemperatureComponentsPage {
  ...
  deleteButtons = element.all(by.css('gp-temperature div table .btn-danger'));
  title = element.all(by.css('gp-temperature div h2#page-heading span')).first();
...
export class TemperatureUpdatePage {
  pageTitle = element(by.id('gp-temperature-heading'));
...
export class TemperatureDeleteDialog {
  private dialogTitle = element(by.id('gp-delete-temperature-heading'));
  private confirmButton = element(by.id('gp-confirm-delete-temperature'));
