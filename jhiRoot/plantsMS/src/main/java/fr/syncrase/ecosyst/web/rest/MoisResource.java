...
public class MoisResource {
    ...
    private static final String ENTITY_NAME = "plantsMsMois";
    ...
    public ResponseEntity<Mois> updateMois(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Mois mois)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, mois.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Mois> partialUpdateMois(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, mois.getId().toString())
        );
    }
     ...
     */
    @GetMapping("/mois")
    public ResponseEntity<List<Mois>> getAllMois(MoisCriteria criteria, Pageable pageable) {
