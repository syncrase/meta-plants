...
class ClassificationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Classification entity")
        ...
        .repeat(2) {
            ...
            .get("/services/plantsms/api/classifications")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new classification")
            .post("/services/plantsms/api/classifications")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/services/plantsms${new_classification_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created classification")
            .delete("/services/plantsms${new_classification_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
