...
class RacineResourceIT {
    ...
    void createRacine() throws Exception {
        ...
        // Create the Racine
        RacineDTO racineDTO = racineMapper.toDto(racine);
        restRacineMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(racineDTO))
            )
    ...
    void putNewRacine() throws Exception {
        ...
        RacineDTO racineDTO = racineMapper.toDto(updatedRacine);

        restRacineMockMvc
            .perform(
                put(ENTITY_API_URL_ID, racineDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(racineDTO))
            )
    ...
    void patchNonExistingRacine() throws Exception {
        ...
        restRacineMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, racineDTO.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(racineDTO))
            )
    ...
    void deleteRacine() throws Exception {
        ...
        // Delete the racine
        restRacineMockMvc
            .perform(delete(ENTITY_API_URL_ID, racine.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
