...
/**
 * Performance test for the APGIII entity.
 */
class APGIIIGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the APGIII entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all aPGIIIS")
            .get("/services/microservice/api/apgiiis")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new aPGIII")
            .post("/services/microservice/api/apgiiis")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "ordre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_aPGIII_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created aPGIII")
                .get("/services/microservice${new_aPGIII_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created aPGIII")
            .delete("/services/microservice${new_aPGIII_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
