...
/**
 * Performance test for the Raunkier entity.
 */
class RaunkierGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Raunkier entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all raunkiers")
            .get("/services/microservice/api/raunkiers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new raunkier")
            .post("/services/microservice/api/raunkiers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "type":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_raunkier_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created raunkier")
                .get("/services/microservice${new_raunkier_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created raunkier")
            .delete("/services/microservice${new_raunkier_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
