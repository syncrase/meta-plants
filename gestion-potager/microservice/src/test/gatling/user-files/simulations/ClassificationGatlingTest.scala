...
class ClassificationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Classification entity")
        ...
        .repeat(2) {
            ...
            .get("/services/microservice/api/classifications")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new classification")
            .post("/services/microservice/api/classifications")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            ...
            .repeat(5) {
                ...
                .get("/services/microservice${new_classification_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created classification")
            .delete("/services/microservice${new_classification_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
