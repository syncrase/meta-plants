...
/**
 * Performance test for the APGI entity.
 */
class APGIGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the APGI entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all aPGIS")
            .get("/services/microservice/api/apgis")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new aPGI")
            .post("/services/microservice/api/apgis")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "ordre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_aPGI_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created aPGI")
                .get("/services/microservice${new_aPGI_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created aPGI")
            .delete("/services/microservice${new_aPGI_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
