...
class ExpositionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Exposition entity")
        ...
        .repeat(2) {
            ...
            .get("/services/microservice/api/expositions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new exposition")
            .post("/services/microservice/api/expositions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "valeur":"SAMPLE_TEXT"
                , "ensoleilement":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_exposition_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created exposition")
                .get("/services/microservice${new_exposition_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created exposition")
            .delete("/services/microservice${new_exposition_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
