...
/**
 * Performance test for the APGII entity.
 */
class APGIIGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the APGII entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all aPGIIS")
            .get("/services/microservice/api/apgiis")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new aPGII")
            .post("/services/microservice/api/apgiis")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "ordre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_aPGII_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created aPGII")
                .get("/services/microservice${new_aPGII_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created aPGII")
            .delete("/services/microservice${new_aPGII_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
