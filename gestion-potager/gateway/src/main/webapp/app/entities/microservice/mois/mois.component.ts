...
export class MoisComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.moisService
      ...
      .subscribe(
        (res: HttpResponse<IMois[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IMois): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInMois(): void {
    this.eventSubscriber = this.eventManager.subscribe('moisListModification', () => this.loadPage());
  }
