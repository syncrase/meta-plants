...
export class FeuillageComponentsPage {
  ...
  deleteButtons = element.all(by.css('gp-feuillage div table .btn-danger'));
  title = element.all(by.css('gp-feuillage div h2#page-heading span')).first();
...
export class FeuillageUpdatePage {
  pageTitle = element(by.id('gp-feuillage-heading'));
...
export class FeuillageDeleteDialog {
  private dialogTitle = element(by.id('gp-delete-feuillage-heading'));
  private confirmButton = element(by.id('gp-confirm-delete-feuillage'));
