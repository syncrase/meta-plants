...
export class SemisComponentsPage {
  ...
  deleteButtons = element.all(by.css('gp-semis div table .btn-danger'));
  title = element.all(by.css('gp-semis div h2#page-heading span')).first();
...
export class SemisUpdatePage {
  pageTitle = element(by.id('gp-semis-heading'));
...
export class SemisDeleteDialog {
  private dialogTitle = element(by.id('gp-delete-semis-heading'));
